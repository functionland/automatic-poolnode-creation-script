...
get_aws_token() {
    echo $(curl -H "Metadata-Flavor: Google" http://metadata.google.internal/computeMetadata/v1/project/project-id -s)
}

# Function to get the AWS Region
get_aws_region() {
    # In Google Cloud, fetch the zone and trim to get the region
    local region
    region=$(curl -H "Metadata-Flavor: Google" http://metadata.google.internal/computeMetadata/v1/instance/zone -s | rev | cut -d'/' -f1 | rev | sed 's/-[a-z]$//')
    echo "$region"
}
...
zip_and_upload() {
    ...
    echo "Uploading $zip_filename to Google Cloud Storage..."
    # Ensure that gsutil is authenticated and configured
    gsutil cp "$zip_filename" "gs://pools-gc/$zip_filename"
...
get_public_addr() {
    # Directly fetch the instance's public IP
    echo $(curl -H "Metadata-Flavor: Google" http://metadata.google.internal/computeMetadata/v1/instance/network-interfaces/0/access-configs/0/external-ip -s)
}
